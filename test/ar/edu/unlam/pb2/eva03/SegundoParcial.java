...
public class SegundoParcial {
	...
	public void queSePuedaEmitirUnaPolizaDeAutos() {
		...
		assertEquals(NUMERO_DE_POLIZA, poliza.getNumeroDePoliza()); //getNumeroDePoliza estaba sin () pero lo considero metodo get
	...
	public void queSePuedaEmitirUnaPolizaDeVida() {
		...
		assertEquals((Integer)2, poliza.obtenerCantidadDeBeneficiarios());//hacia falta ; y casteo (Integer)
		assertEquals(asegurado, poliza.getAsegurado());
		assertEquals(PREMIO, poliza.getPremio()); // pedia poliza.getAsegurado() pero es poliza.getPremio()
	}
	...
	public void queSePuedaEmitirUnaPolizaDeCombinadoFamiliar() {
		...
		assertEquals((Integer)3, poliza.obtenerCantidadDeBeneficiarios()); // correccion es obtenerCantidadDeBeneficiarios en plural
		assertEquals(asegurado, poliza.getAsegurado());
		assertEquals(PREMIO, poliza.getPremio());
	}
	...
	public void queSePuedanAdministrarDistintosTiposDePolizas() {
		...
		libra.agregarPoliza(new PolizaDeAuto(4, new Persona("Victor", 652354, 24), 2000000.0, 5000.0)); //no debe entrar porque repite numero de poliza 4
		
		assertEquals((Integer)4, libra.obtenerLaCantidadDePolizasEmitidas()); //obtenerLaCantidadDePolizasEmitidas lo consideo metodo get, agregue los ()
	}
	...
	public void queSePuedaDenunciarElRoboDeUnAuto() {
		...
		}catch( PolizaInexistente pi) {
			System.err.println("Poliza no encontrada");
		}
		try {
	...
	@Test
	public void queSePuedaDenunciarUnSiniestroParaUnaPolizaDeVida() { //faltaba un (
		Integer numeroDePoliza = 1;
		CompaniaDeSeguro libra = new CompaniaDeSeguro("Libra");
	...
	public void verificarQueNoSePuedaDenunciarUnSiniestroParaUnaPolizaQueNoExista() throws PolizaInexistente { //faltaban los parentesis de cierre
		Integer numeroDePoliza = 1;
		CompaniaDeSeguro libra = new CompaniaDeSeguro("Libra");
